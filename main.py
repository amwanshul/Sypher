...
from tools.catalog import TOOL_DECLARATIONS as REGISTERED_TOOL_DECLARATIONS, TOOL_REGISTRY
...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are Sypher, a sharp desktop AI assistant. "
...
# Legacy backup only. Runtime declarations now come from tools/schemas.json.
...
TOOL_DECLARATIONS = REGISTERED_TOOL_DECLARATIONS
                    ...
                    if response.server_content:
                        ...
                        if sc.turn_complete:
                            ...
                            if out_buf:
                                ...
                                if full_out:
                                    self.ui.write_log(f"Sypher: {full_out}")
                            out_buf = []
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    self.ui.write_log("Sypher online.")
...
async def _registry_execute_tool(self, fc) -> types.FunctionResponse:
    name = fc.name
    args = dict(fc.args or {})
    print(f"[JARVIS] TOOL: {name} ARGS: {args}")

    try:
        result = await TOOL_REGISTRY.execute_live(
            name,
            args,
            player=self.ui,
            speak=self.speak,
...
async def _resilient_listen_audio(self):
    while True:
        stream = None
        try:
            print("[JARVIS] Mic started")
            ...
            print(f"[JARVIS] Mic error: {e}")
            self.ui.write_log("SYS: Microphone stream lost. Retrying...")
...
async def _resilient_play_audio(self):
    while True:
        stream = None
        try:
            print("[JARVIS] Play started")
            ...
            print(f"[JARVIS] Play error: {e}")
            self.ui.write_log("SYS: Speaker stream lost. Retrying...")
            await asyncio.sleep(2)
        finally:
            if stream is not None:
                try:
...
JarvisLive._execute_tool = _registry_execute_tool
JarvisLive._listen_audio = _resilient_listen_audio
JarvisLive._play_audio = _resilient_play_audio
