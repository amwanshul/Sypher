import os, json, time, math, random, threading, queue
...
class JarvisUI:
    ...
    def _draw(self):
        ...
        c.create_text(W // 2, 44, text="Voice-Driven Desktop Assistant",
        ...
        c.create_text(W // 2, H - 14, fill=C_DIM, font=("Courier", 8),
                      text="FatihMakes Industries  ·  CLASSIFIED  ·  SYPHER")
    ...
    def _process_ui_queue(self):
        while True:
            ...
            action = item["action"]
            if action == "log":
                self._write_log_main_thread(item["text"])
            elif action == "speaking_state":
                self.speaking = item["speaking"]
                self.status_text = "SPEAKING" if self.speaking else "ONLINE"
            elif action == "confirm":
                result = messagebox.askyesno(
                    item["title"],
                    item["message"],
                    parent=self.root,
                    default=messagebox.YES if item["default"] else messagebox.NO,
                )
    ...
    def _dispatch_ui(self, action: str, *, wait: bool = False, timeout_s: float = 120.0, **payload):
        if threading.get_ident() == self._ui_thread_id and not wait:
            if action == "log":
                self._write_log_main_thread(payload["text"])
            elif action == "speaking_state":
        ...
        if not event.wait(timeout_s):
            return payload.get("default", False)
        return bool(result.get("value", payload.get("default", False)))
    ...
    def write_log(self, text: str):
        self._dispatch_ui("log", text=text)
    ...
    def start_speaking(self):
        self._dispatch_ui("speaking_state", speaking=True)

    def stop_speaking(self):
        self._dispatch_ui("speaking_state", speaking=False)

    def confirm_action(self, title: str, message: str, default: bool = False, timeout_s: float = 120.0) -> bool:
    ...
    def wait_for_api_key(self):
        """Block until API key is saved (called from the runtime thread before starting Sypher)."""
    ...
    def _show_setup_ui(self):
        ...
        tk.Label(self.setup_frame,
                 text="Enter your Gemini API key to boot Sypher.",
    ...
    def _save_api_keys(self):
        ...
        self.status_text = "ONLINE"
        self.write_log("SYS: Systems initialised. Sypher online.")
