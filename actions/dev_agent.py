...
PROJECTS_DIR       = Path.home() / "Desktop" / "SypherProjects"
MAX_FIX_ATTEMPTS   = 4
...
Code for {file_path}:"""
    ...
    try:
        ...
        # Save file
        full_path = ctx.project_dir / file_path
...
def _approval_blocked(message: str) -> bool:
    prefixes = (
        "Untrusted code execution is disabled",
        "Execution requires a confirmation-capable UI",
        "Execution cancelled",
    )
...
def _run_project(run_command: str, project_dir: Path, timeout: int = 30, player=None) -> str:
    """Run the project entry point, return output."""
    print(f"[DevAgent] 🚀 Running: {run_command}")

    approved, message = require_untrusted_execution_approval(
        player=player,
        source="dev_agent.run_project",
        reason=f"Run generated project in {project_dir}",
        code_preview=run_command,
    )
    if not approved:
        return message
...
def _build_project(
    ...
    proj_name = project_name or plan.get("project_name", "sypher_project")
    ...
    if dependencies:
        ...
        approved, message = require_untrusted_execution_approval(
            player=player,
            source="dev_agent.install_dependencies",
            reason=f"Install dependencies for generated project in {project_dir}",
            code_preview="python -m pip install " + " ".join(dependencies),
        )
        if not approved:
            if speak:
                speak(message)
    ...
    for attempt in range(1, MAX_FIX_ATTEMPTS + 1):
        ...
        last_output = _run_project(run_command, project_dir, timeout, player=player)
        if _approval_blocked(last_output):
            if speak:
                speak(last_output)
            return last_output
