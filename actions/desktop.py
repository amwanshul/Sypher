...
def _execute_generated_code(code: str, *, player=None, reason: str = "") -> str:
    ...
    if not safe:
        return f"⛔ Blocked for safety: {safety_reason}"

    approved, message = require_untrusted_execution_approval(
        player=player,
        source="desktop_control.task",
        reason=reason or "Run AI-generated desktop automation code.",
        code_preview=code,
    )
    if not approved:
        return message
    ...
    try:
        if action == "wallpaper":
            ...
            if code == "UNSAFE":
                ...
                result = _execute_generated_code(code, player=player, reason=actual_task)
            ...
            if full_task:
                ...
                result = (
                    _execute_generated_code(code, player=player, reason=full_task)
