...
def _resolve_save_path(output_path: str, language: str) -> Path:
    ...
    return DESKTOP / f"sypher_code{ext}"
...
def _take_screenshot() -> Path | None:
    try:
        import pyautogui
        screenshot_path = Path.home() / "Desktop" / f"sypher_debug_{int(time.time())}.png"
...
def _read_preview(path: Path, limit: int = 1200) -> str:
    try:
        ...
        return f"<preview unavailable for {path.name}>"


def _execution_blocked(message: str) -> bool:
    prefixes = (
...
def _run_file(
    ...
    player=None,
    *,
    approval_source: str = "code_helper.run",
    approval_reason: str = "",
    code_preview: str = "",
) -> str:
    interpreters = {
    ...
    approved, message = require_untrusted_execution_approval(
        ...
        reason=approval_reason or f"Run {path.name}",
        code_preview=code_preview or _read_preview(path),
    )
    if not approved:
        return message
...
def _build(description, language, output_path, args, timeout, speak=None, player=None) -> str:
    ...
    for attempt in range(1, MAX_BUILD_ATTEMPTS + 1):
        ...
        last_output = _run_file(
            path,
            args,
            timeout,
            player,
            approval_source="code_helper.build",
            approval_reason=description or f"Build and run {path.name}",
            code_preview=code,
        )
        if _execution_blocked(last_output):
            if speak:
                speak(last_output)
            return last_output
...
def _run_action(file_path, args, timeout, player) -> str:
    ...
    return _run_file(
        ...
        timeout,
        player,
        approval_source="code_helper.run",
        approval_reason=f"Run local file {p}",
    )
